...
/* ================== STORAGE (READ-ONLY) ==================
   ...
   Do NOT change other pages; Reports just reads.
*/
...
function inRange(dateStr: string, from: string, to: string) {
  ...
  return dateStr >= from && dateStr <= to; // YYYY-MM-DD lexicographic compare
}
...
function ThemeTokens(theme: Theme = "Royal Gold", highContrast?: boolean) {
  ...
  const base = {
    ...
    hoverBg: "rgba(0,0,0,0.45)", // << BLACK HOVER
...
function NavItem({
  ...
  const style: CSSProperties = {
    ...(S.navItem as any),
    ...(hover ? { background: T.hoverBg, border: `1px solid ${T.accentBd}` } : null),
    ...(active ? { border: `1px solid ${T.accentBd}`, background: T.accentBg } : null),
  };
  return (
    <Link
      href={href}
...
export default function ReportsPage() {
  ...
  function exportAllCSV() {
    const rows = [
      ...
      { section: "SUMMARY", item: "Income", value: finTotals.income },
      { section: "SUMMARY", item: "Expense", value: finTotals.expense },
      { section: "SUMMARY", item: "Team", value: hrKpis.total },
      { section: "SUMMARY", item: "Active Staff", value: hrKpis.active },
      { section: "SUMMARY", item: "Vendors", value: vendorKpis.total },
      { section: "SUMMARY", item: "Avg Vendor Rating", value: vendorKpis.avgRating },
      { section: "SUMMARY", item: "High Workload Count", value: hrKpis.highLoad },
    ];
  ...
  return (
    <div style={S.app}>
      <aside style={S.sidebar}>
        ...
        <nav style={S.nav}>
          <NavItem href="/dashboard" label="📊 Dashboard" S={S} T={T} />
          <NavItem href="/events" label="📅 Events" S={S} T={T} />
          <NavItem href="/finance" label="💰 Finance" S={S} T={T} />
          <NavItem href="/vendors" label="🏷️ Vendors" S={S} T={T} />
          <NavItem href="/hr" label="🧑‍🤝‍🧑 HR" S={S} T={T} />
          <NavItem href="/reports" label="📈 Reports" active S={S} T={T} />
          <NavItem href="/settings" label="⚙️ Settings" S={S} T={T} />
        </nav>
      ...
      <main style={S.main}>
        ...
        <div style={S.grid}>
          ...
          <section style={S.panel}>
            ...
            {/* IMPORTANT: uses ≥ to avoid Turbopack parsing error */}
