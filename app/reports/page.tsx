...
// Try common keys you may have used in other pages/snippets
...
function inRange(dateStr: string, from: string, to: string) {
  ...
  return dateStr >= from && dateStr <= to; // for YYYY-MM-DD
}
...
function normalizeEvents(raw: any): NormalEvent[] {
  ...
  return arr
    .map((x) => {
      ...
      const date = String(x?.date ?? x?.eventDate ?? x?.startDate ?? "");
...
function normalizeFinance(raw: any): NormalTx[] {
  ...
  return arr
    .map((x) => {
      const id = String(x?.id ?? x?._id ?? "");
      const date = String(x?.date ?? x?.txDate ?? x?.createdAt?.slice?.(0, 10) ?? "");
      const tRaw = String(x?.type ?? "").toLowerCase();
      const type: "Income" | "Expense" = tRaw === "income" ? "Income" : "Expense";
...
export default function ReportsPage() {
  ...
  const [keysInfo, setKeysInfo] = useState<{
    ...
    hr: string | null;
    vendors: string | null;
  }>({ events: null, fin: null, hr: null, vendors: null });
  ...
  // load settings + module data (reads multiple keys; never breaks)
  useEffect(() => {
  ...
  const execSummary = useMemo(() => {
    ...
    if (finTotals.net < 0) lines.push(`⚠ Net is negative in this range. Focus on expense control + collections.`);

    const noData = !events.length && !txs.length && !team.length && !vendors.length;
    if (noData) lines.push(`No module data found. Add at least one entry in each module to see reports.`);

    return lines;
  }, [from, to, eventsInRange.length, eventStatus, finTotals, topExp, topInc, hrKpis, vendorKpis, team, events.length, txs.length, vendors.length]);
  ...
  function exportAllJSON() {
    ...
    exportJSON(`eventura_company_report_${from}_to_${to}.json`, payload);
  }
      ...
      <main style={S.main}>
        <div style={S.header}>
          <div>
            ...
            <div style={S.muted}>
              Company overview • Logged in as <b>{email || "Unknown"}</b> • Role:
          ...
          <div style={S.headerRight}>
            ...
            {isCEO ? (
              <>
                ...
                <button style={S.secondaryBtn} onClick={() => exportCSV(`eventura_hr_${todayISO()}.csv`, team as any)}>
                  Export HR CSV
                </button>
                <button style={S.secondaryBtn} onClick={() => exportCSV(`eventura_vendors_${todayISO()}.csv`, vendors as any)}>
                  Export Vendors CSV
                </button>
              </>
            ) : null}
        ...
        <div style={S.rangeBar}>
          ...
          <div style={S.smallNote}>
            Tip: Reports uses date fields (YYYY-MM-DD). Finance/Events must store valid dates; HR/Vendors still show overall KPIs even without dates.
          </div>
        </div>
        ...
        <div style={S.grid}>
          ...
          <section style={S.panel}>
            ...
            <div style={S.smallNote}>
              KPIs are for selected date range where possible. HR/Vendors are overall because those records usually don’t carry dates.
            </div>
          </section>
                    ...
                    .map((e) => (
                      <div key={e.id} style={S.itemCard}>
                        ...
                        <div style={S.smallMuted}>
                          {e.date} • {e.city || "—"} {typeof e.budget === "number" ? `• Budget ₹${inr(e.budget)}` : ""}
                        </div>
                      </div>
                    ))}
                </div>
          ...
          <section style={S.panel}>
            <div style={S.panelTitle}>Finance</div>
            <div style={S.kpiRow}>
          ...
          <section style={S.panel}>
            <div style={S.panelTitle}>Vendors</div>
            <div style={S.kpiRow}>
        ...
        <div style={S.footerNote}>✅ Reads existing module data • ✅ No changes to other pages • ✅ Export JSON/CSV</div>
      </main>
    </div>
  );
}
