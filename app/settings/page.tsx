    ...
    <div style={S.app}>
      <aside style={S.sidebar}>
        ...
        <nav style={S.nav}>
          ...
          <Link href="/settings" style={S.navActive as any}>⚙️ Settings</Link>
        </nav>
        ...
        <div style={S.sidebarFooter}>
          ...
          <div style={S.smallNote}>Settings restored to unblock deployment.</div>
        </div>
      </aside>
      ...
      <main style={S.main}>
        <div style={S.header}>
          <div>
            <div style={S.h1}>Settings (Stable)</div>
            <div style={S.muted}>Theme + contrast preferences (stored locally).</div>
          </div>
        </div>
        ...
        <section style={S.panel}>
          ...
          <div style={S.grid}>
            <div style={S.field}>
              <div style={S.smallMuted}>Theme</div>
              <select
                style={S.select}
                value={(settings.theme as Theme) || "Royal Gold"}
                onChange={(e) => {
                  update({ ...settings, theme: e.target.value as Theme });
                  toast("✅ Theme saved");
                }}
              >
                ...
                <option value="Ruby Noir">Ruby Noir</option>
            ...
            <div style={S.field}>
              <div style={S.smallMuted}>High Contrast</div>
              <button
                style={S.btn}
                onClick={() => {
            ...
            <div style={S.field}>
              <div style={S.smallMuted}>Compact Tables</div>
              <button
                style={S.btn}
                onClick={() => {
                  update({ ...settings, compactTables: !settings.compactTables });
                  toast("✅ Saved");
                }}
              >
          ...
          <div style={S.smallNote}>
            CEO Email (for role detection): <b>{settings.ceoEmail || "hardikvekariya799@gmail.com"}</b>
          </div>
        </section>
      </main>
    </div>
...
function makeStyles(T: any): Record<string, CSSProperties> {
  return {
    ...
    userBox: { padding: 12, borderRadius: 16, border: `1px solid ${T.border}`, background: "rgba(255,255,255,0.04)" },
    ...
    header: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, padding: 14, borderRadius: 18, border: `1px solid ${T.border}`, background: "rgba(11,16,32,0.60)", backdropFilter: "blur(10px)" },
    h1: { fontSize: 26, fontWeight: 950 },
    muted: { color: T.muted, fontSize: 13, marginTop: 6 },
    panel: { marginTop: 12, padding: 14, borderRadius: 18, border: `1px solid ${T.border}`, background: "rgba(11,16,32,0.60)", backdropFilter: "blur(10px)" },
    panelTitle: { fontWeight: 950, color: T.accentTx },
    msg: { marginTop: 12, padding: 10, borderRadius: 14, border: `1px solid ${T.border}`, background: "rgba(255,255,255,0.04)", color: T.text, fontSize: 13 },
    grid: { marginTop: 12, display: "grid", gridTemplateColumns: "280px 280px 280px", gap: 12, alignItems: "end" },
    field: { display: "grid", gap: 6 },
    smallMuted: { color: T.muted, fontSize: 12 },
    select: { width: 280, padding: "12px 12px", borderRadius: 14, border: `1px solid ${T.border}`, background: "rgba(255,255,255,0.06)", color: T.text, outline: "none", fontSize: 14 },
    btn: { width: 280, padding: "12px 12px", borderRadius: 14, border: `1px solid ${T.border}`, background: "rgba(255,255,255,0.04)", color: T.text, fontWeight: 950, cursor: "pointer" },
  };
}
